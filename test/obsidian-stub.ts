...
// --- vault primitives ---------------------------------------------------------------------
//
// The tracker narrows every event's payload with `instanceof TFile` before it records
// anything (a `file-open` fires for non-file items too). esbuild aliases `obsidian` to this
// stub for the code under test AND for the test, so both see the SAME class and the guard
// behaves exactly as it does in Obsidian. Without a real class here, every guard would reject
// every event and an integration test would "pass" against a plugin that records nothing.
...
/** Obsidian's own: collapse duplicate slashes, strip a leading/trailing one. */
