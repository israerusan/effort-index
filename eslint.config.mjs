 ...
 *
 * `src/shared/**` is IGNORED here on purpose: the vendored core is linted once, in
 * obsidian-plugin-core, not five times over — and it must never be "fixed" in a plugin
 * repo, which is how two divergent copies of verifyLicense.mjs happened. It is still
 * TYPECHECKED (tsc covers src/**\/*.ts), which is the drift alarm for the hand-written
 * .d.mts files.
 */
export default tseslint.config(
	...
	// Re-enable type-aware linting on the source (the obsidianmd recommended set ships with
	// type-checked linting disabled). Scoped to src/**/*.ts so the JS config files above are
	// never parsed with type info.
	{
		files: ["src/**/*.ts"],
		languageOptions: {
			parserOptions: {
		...
		rules: {
			...
			// ("Effort Index", "Second Read", "Pro") — lowercasing them would be wrong, and the
